...
def get_comments_by_phrase():
    ...
    books = Feedback.query.filter(Feedback.title.ilike(f"%{phrase}%")).all()
    return jsonify([{"id": book.id, "title": book.title, "author": book.author, "year": book.year} for book in books]), 200

# API Query2 Retrieve comments containing a specific phrase 'done'
...
def get_books_by_author_length():
    ...
    books = Feedback.query.filter(func.length(Feedback.author) >= min_length).all()
    return jsonify([{"id": book.id, "title": book.title, "author": book.author, "year": book.year} for book in books]), 200
...
def update_multiple_books():
    ...
    books = Feedback.query.filter(Feedback.id.in_(ids)).all()
    for book in books:
        book.year = new_year
    db.session.commit()
